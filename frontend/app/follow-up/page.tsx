...
import { FUContext } from './contexts'
...
export default function Home() {
  ...
  // meeting data
  const followUp = useSynced("FOLLOW_UP", {
    caseData: {} as any,
    meetings: [] as any[],
    selectedMeeting: null as number | null,
    chatEnded: false,
  })
  const { caseData, meetings, selectedMeeting, setSelectedMeeting, chatEnded } = followUp
  ...
  return (
    <FUContext.Provider value={followUp}>
      <SidebarLayout
      ...
      >
        <Chat
          ...
          history={
            selectedMeeting && selectedMeeting == meetings[meetings.length - 1].timestamp
              ? (messages.partial
                ...
                : messages.history)
              : meetings.find(({ timestamp }: any) => timestamp == selectedMeeting)?.chat || []
          }
          onSend={
            (message) => {
          ...
          isDisabled={!selectedMeeting || selectedMeeting != meetings[meetings.length - 1].timestamp || chatEnded}
          showSystem={showSystem}
        />
      </SidebarLayout>
    </FUContext.Provider>
